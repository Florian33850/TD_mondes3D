...
void Mesh::computeNormals()
{
  ...
  for(unsigned int i = 0; i <  mFaces.size(); i++)
  {
    ...
    p0.normal+= q1.cross(q2);
    p1.normal+= q1.cross(q2);
    p2.normal+= q1.cross(q2);
    ...
    Matrix<float,3,2> TB = M * tex.inverse();
  ...
  for(Vertex& vertex : mVertices)
  {
    vertex.tangent = vertex.tangent - vertex.normal.dot(vertex.tangent) * vertex.normal;
    vertex.bitangent = vertex.bitangent - vertex.normal.dot(vertex.bitangent) * vertex.normal - vertex.tangent.dot(vertex.bitangent) * vertex.tangent/vertex.tangent.norm();
...
void Mesh::draw(const Shader &shd)
{
  ...
  //Introduction
  int textureCoord_loc = shd.getAttribLocation("vtx_text_coord");
  if(textureCoord_loc>=0)
  {
    glVertexAttribPointer(textureCoord_loc, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex), (void*)(2*sizeof(Vector3f)+sizeof(Vector4f)));
  ...
  if(textureCoord_loc>=0)  glDisableVertexAttribArray(textureCoord_loc);
