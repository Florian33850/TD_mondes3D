...
void Viewer::init(int w, int h){
    ...
    if(!_cylinder.load(DATA_DIR"/models/cylinder.obj")) exit(1);
    _cylinder.init();
...
void Viewer::drawScene()
{
    ...
    Affine3f M2(AngleAxisf(_theta,Vector3f(0,1,0)));

    glUniformMatrix4fv(_shader2.getUniformLocation("obj_mat"),1,GL_FALSE,M2.matrix().data());
    ...
    lightDir = Vector3f(1,0,1).normalized();
...
void Viewer::updateAndDrawScene()
{
  if(_IK_target.norm()>0)
  {
    ...
    for(int i=0; i<n; ++i)
    {
      ...
      float phi = _jointAngles(0, i);
      float theta = _jointAngles(1, i);
      M = M*AngleAxisf(phi, Vector3f::UnitZ())*AngleAxisf(theta, Vector3f::UnitY())*T;
    }
    ...
    for(int i=0; i<n; ++i)
    {
      ...
      M = M*AngleAxisf(phi, Vector3f::UnitZ());
      ri = extremite - M.translation();//ri = centre de rotation vers extrémité courante
      wi0 = M.linear().col(2);//Wi = Z
      J.col(indice) = wi0.cross(ri);
      indice++;

      M = M*AngleAxisf(theta, Vector3f::UnitY());
      ri = extremite - M.translation();
      wi1 = M.linear().col(1);//Wi+1 = Y
