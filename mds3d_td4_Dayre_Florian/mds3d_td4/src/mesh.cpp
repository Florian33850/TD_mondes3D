...
{
  // TODO
  //Mettre à 0 les normales des sommets 
  for(Vertex& vertex:mVertices)
    vertex.normal = Vector3f(0,0,0);

  //Pour chaque face, calculer sa normale (sans la normaliser) et l'ajouter à la normales des sommets de la face
  for(unsigned int i = 0; i <  mFaces.size(); i++)
  {
    Vector3i face = mFaces[i];

    Vector3f pos1 = mVertices[face.x()].position - mVertices[face.z()].position;
    Vector3f pos2 = mVertices[face.y()].position - mVertices[face.z()].position;

    mVertices[face.x()].normal+= pos1.cross(pos2);
    mVertices[face.y()].normal+= pos1.cross(pos2);
    mVertices[face.z()].normal+= pos1.cross(pos2);
  }
