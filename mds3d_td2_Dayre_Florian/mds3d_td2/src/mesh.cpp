...
bool Mesh::intersectFace(const Ray& ray, Hit& hit, int faceId) const
{
    ...
    float a = (T.cross(E1)).dot(E2);
    float b = (D.cross(E2)).dot(T);
    float c = (T.cross(E1)).dot(D);
    Point3f m = {a, b, c};

    Point3f intersect = (1/((D.cross(E2)).dot(E1)))*m;

    float t = intersect[0];
    float u = intersect[1];
    float v = intersect[2];

    if(t>0 && u>=0 && u<=1 && v>=0 && v<=1 && (u+v)>=0 && (u+v)<=1){
        hit.setT(t);

        float lambda1 = u;
        float lambda2 = v;
        float lambda3 = 1-lambda1-lambda2;
        Normal3f normal = lambda1*this->vertexOfFace(faceId, 0).normal + lambda2*this->vertexOfFace(faceId, 1).normal + lambda3*this->vertexOfFace(faceId, 2).normal;
        normal.normalize();
